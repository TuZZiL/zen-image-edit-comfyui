...
| вхід | значення |
|---|---|
| `adapter_file` | **дропдаун** зі `.safetensors`-файлів у твоїх теках `models/`, з **кнопкою завантаження** для того, якого ще немає (архітектура, включно з шириною таблиці позицій, читається з його метаданих) |
| `text_encoder` | **дропдаун** з тек усередині `models/text_encoders/`, плюс repo id `Qwen/Qwen3.5-0.8B` |
...
Обидва файлові входи — це **вибір із списку**, а не текстові поля. Раніше треба було
вписувати абсолютні шляхи руками, і це була найчастіша причина невдалого налаштування.
Рядок із старого воркфлоу чи з API при цьому досі приймається без проблем.
...
`ZenImage21AdapterLoader` / `ZenImage21TextEncode`. Якщо поставити обидві копії без змін, **одна з
них тихо замінить іншу** в меню нод — і ти не скажеш, яку саме реалізацію використовує воркфлоу.

Хто саме переможе, вирішує завантажувач ComfyUI, а не порядок встановлення. Він обходить
`os.listdir(custom_node_path)` **без сортування** (`nodes.py`, `init_external_custom_nodes`), а потім
присвоює `NODE_CLASS_MAPPINGS[schema.node_id] = node_cls`, тож **id забирає остання директорія в
порядку `listdir`**. Цей порядок визначає файлова система: на tmpfs перемагає встановлена першою
копія, на ext4 діє хешований порядок директорій із per-filesystem seed, на NTFS — ім'я. Тому
перейменування теки клону може змінити, яку ноду отримає воркфлоу. (Виміряно на tmpfs/ext4 агентом
`mira` на GetPostingBoard, seq 59414; рядок для NTFS додано на seq 59437 — а правило, на яке цей
репозиторій спирається замість цього, зафіксовано в `tests/test_node_suffix.py`.)
...
### Як підписати другий чекаут **самого форку**

`ZEN_NODE_SUFFIX` читається один раз на імпорті, а всі чекаути імпортуються в одному процесі
ComfyUI — тож **сама лише env-змінна не розділить два чекаути форку**: обидва читають те саме
оточення й беруть той самий суфікс. Назви кожен чекаут файлом поряд із цим README:

```bash
echo MyFork > .zen_node_suffix     # id стануть …LoaderMyFork / …TextEncodeMyFork
```

Порядок вирішення, від старшого:

1. `ZEN_NODE_SUFFIX` — явне перекриття; `""` означає upstream-сумісні id (без змін)
2. `<repo>/.zen_node_suffix` — ім'я чекауту, окремий файл на кожен клон
3. `"Plus"` — дефолт вище

Порожній файл вважається відсутнім, а не проханням прибрати суфікс.

Кожна реєстрація також пише в лог, яка саме директорія віддала ці id — тож «яка реалізація
працює» вирішується одним grep по стартовому логу:
